...
const fetchCities = province => {
    return new Promise(resolve => {
        setTimeout(() => {
            resolve(
                {
                    beijing: [{ name: '朝阳', key: 'chaoyang' }, { name: '海淀', key: 'haidian' }],
                    shanghai: [{ name: '浦东', key: 'pudong' }, { name: '徐汇', key: 'xuhui' }],
                    jiangsu: [{ name: '南京', key: 'nanjing' }, { name: '苏州', key: 'suzhou' }],
                    shandong: [{ name: '青岛', key: 'qingdao' }, { name: '德州', key: 'dezhou' }]
                }[province]
            );
        }, 1000);
    });
            ...
            elements: _.compact(
                formMeta.elements.map(m => {
                    ...
                    if (m.key === 'province' || m.key === 'city') {
                        const values = this.state[m.key === 'province' ? 'provinces' : 'cities'];
                        return {
                            ...m,
                            widget: Select,
                            initialValue: values && userData && userData[m.key] ? userData[m.key] : 'loading',
                            children: values
